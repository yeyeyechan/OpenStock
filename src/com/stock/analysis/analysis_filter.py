...
def check_layer(data, layer):
    ...
    if not ("단계" in daily_check_data.keys()) :
...
def analysis (tr_name , data):
    if tr_name == "SC" :
        if check_time(data["체결시간"], "0905"):
            ...
            SK_data = SK.find_one({"단축코드" : data["단축코드"]}, sort=[("체결시간", pymongo.DESCENDING)])
            SP_data = SP.find_one({"단축코드" : data["단축코드"]}, sort=[("체결시간", pymongo.DESCENDING)])
            ...
            if data["전일대비구분"] == "2" and int(data["저가"]) > int(before_TR_1206_data["가격"]) and int(data["전일대비율"]) >= 0.7 and  int(data["전일대비율"]) <= 2.0  :
                ...
                if "외국계순매수수량" in SK_data.keys() and  SK_data["외국계순매수수량"] > 0 :
                    if check_layer(data, "2"):
                        send_telegram(data["단축코드"] + "  2단계 조건 달성  ")
                    if "비차익매수위탁체결수량" in SP_data.keys()  and int(SP_data["비차익매수위탁체결수량"]) - int(SP_data["비차익매도위탁체결수량"]) > 0 :
